      ...
      switch (userRole) {
        case 'administrador':
          ...
          if (state.url.includes('/administrador')) {
            ...
            this.router.navigate(['login']); // Redirigir a la ruta de pedidos si intenta acceder a otras rutas
            return false;
          }
        case 'alumno':
          // Si el usuario es 'chef', permitir acceso a la ruta de cocina
          if (state.url.includes('/alumno')) {
            return true;
          } else {
            this.router.navigate(['login']); // Redirigir a la ruta de cocina si intenta acceder a otras rutas
            return false;
          }
          case 'profesor':
          // Si el usuario es 'chef', permitir acceso a la ruta de cocina
          if (state.url.includes('/profesor')) {
            return true;
          } else {
            this.router.navigate(['login']); // Redirigir a la ruta de cocina si intenta acceder a otras rutas
            return false;
